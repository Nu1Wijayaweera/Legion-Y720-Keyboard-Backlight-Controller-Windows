...
/* uninstall.c - Uninstall helper utilities
 *
 * Purpose:
 * - Provide a small, self-contained helper that schedules an uninstall batch
 *   and cleans up configuration/state files and the startup entry. The module
 *   is intentionally best-effort and defensive to avoid accidental data loss.
 *
 * Organization:
 * - Small internal helpers (safe path builders, config/state path helpers)
 * - Startup entry removal and cleanup logic
 * - uninstall_schedule_and_cleanup: public function called by the GUI
 *
 * The implementation keeps copies of simple helpers to remain self-contained
 * and easy to inspect for students learning Win32 programming.
 */

/* === INTERNAL HELPERS ===
 * Small helpers that perform path handling and basic validation. Kept private
 * to avoid depending on unrelated modules and to make the runtime flow clear.
 */
...
static int uc_get_config_path(char *buffer, DWORD buffer_size)
{
    ...
    if (!uc_get_exe_directory(directory, sizeof(directory)))
        goto fallback;
    if (!uc_build_path(candidate, sizeof(candidate), directory, "Y720Backlight.ini"))
        goto fallback;
...
fallback:
    ...
    if (!length || length >= sizeof(appdata) || !appdata[0])
        return 0;
    if (!uc_build_path(folder, sizeof(folder), appdata, "LegionY720Backlight"))
        return 0;
    CreateDirectoryA(folder, NULL);
    if (!uc_build_path(fallback, sizeof(fallback), folder, "Y720Backlight.ini"))
        return 0;
...
static int uc_get_state_path(char *buffer, DWORD buffer_size)
{
    ...
    if (!length || length >= sizeof(appdata) || !appdata[0])
        return 0;
    if (!uc_build_path(buffer, buffer_size, appdata, "LegionY720Backlight"))
        return 0;
    CreateDirectoryA(buffer, NULL);
    if (!uc_build_path(buffer, buffer_size, appdata, "LegionY720Backlight\\state.ini"))
        return 0;
    return 1;
}
...
void uninstall_schedule_and_cleanup(HWND owner)
{
    ...
    if (!uc_is_safe_path(exe))
        exe[0] = '\0'; /* Validate exe path before use */
    if (!GetTempPathA(sizeof(temp_path), temp_path))
        temp_path[0] = '\0';
    if (!uc_is_safe_path(temp_path))
        temp_path[0] = '\0'; /* Validate temp path before use */
